...
func NewShowCommand(app *App) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cacheKey := readCacheKey("GET", "/project", projectID, "task", taskID)
			var task dida.Task
			if !readCacheGet(app, cacheKey, &task) {
				fresh, err := client.GetTask(projectID, taskID)
				if err != nil {
					return err
				}
				task = *fresh
				readCachePut(app, cacheKey, task)
			}
			...
			rows := [][]string{
				...
				{"Title", task.Title},
				{"Status", formatStatus(task.Status, task.CompletedTime)},
				{"Completed", completeLabel(task.Status, task.CompletedTime)},
				{"CompletedTime", task.CompletedTime},
				{"AllDay", fmt.Sprintf("%t", task.IsAllDay)},
